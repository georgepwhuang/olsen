...
from olsen.datamod.datamodule import DocumentClassificationDataWithDataAugmentation
from olsen.model.bert_slider_with_uda_sup import BertSliderWithSupervisedUDA
...
parser.add_lightning_class_args(DocumentClassificationDataWithDataAugmentation, "data")
parser.add_lightning_class_args(BertSliderWithSupervisedUDA, "model")
...
parser.set_defaults({"checkpoint.dirpath": str(consts.MODEL_DIR),
                     "data.train_filename": str(TRAIN_PATH.joinpath("sectLabelDocAug.train")),
...
data = DocumentClassificationDataWithDataAugmentation(**vars(args.data))
model = BertSliderWithSupervisedUDA(**vars(args.model))
